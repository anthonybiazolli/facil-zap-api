...
// Create new session
app.post('/sessions', validate(createSessionSchema), sessionController.createSession);
...
app.post(
  ...
  sessionController.sendBulkMessages
);
...
app.patch(
  '/sessions/:sessionId/groups/:groupJid/subject',
  validate(groupSubjectSchema),
  sessionController.updateGroupSubject
);
...
app.get('/metrics', (req, res) => {
  ...
  const metrics = {
    ...
    messages: sessions.reduce(
      (acc, s) => ({
        sent: acc.sent + s.metrics.messagesSent,
        received: acc.received + s.metrics.messagesReceived,
        failed: acc.failed + s.metrics.messagesFailed,
      }),
